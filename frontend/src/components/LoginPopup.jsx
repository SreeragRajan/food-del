...
const LoginPopup = ({setShowLogin}) => {
  ...
  return (
    <div className='h-full w-full absolute z-[99] bg-black/50 flex justify-center items-center overflow-hidden '>
        <form action="" className='flex flex-col gap-4 bg-white p-8 rounded-md w-[350px] login-fadeIn'>
            ...
            <div className='flex flex-col gap-3'>
                {currState === "Login" && 
                    <input className='outline-none p-2 border' type="text" name='name' placeholder='name' />
                }
                <input className='outline-none p-2 border' type="email" name="" placeholder='email' />
                <input className='outline-none p-2 border' type="password" name="password" placeholder='password' />
            </div>
            {currState === "Sign Up" ? <button className='bg-[#FF4C24] text-white p-2'>Create Account</button>:<button className='bg-[#FF4C24] text-white p-2'>Login</button>}
